...
// 1. We will append all missing countries to the end of COUNTRY_PROFILES.
// We look for the last country which is 'china', and append after it.
const toInject = `  },
  japan: {
    id: 'japan',
    name: "Japonya",
...
if (!content.includes('japan: {')) {
  // Find the exact closing pattern for China which is the last country before
  const searchPattern = /    festivals: \{\n\s*title: '🎉 Festivaller',\n\s*desc: 'Geleneksel Çin Yeni Yılı[\s\S]*?\}\n\s*\}\n\};/;
  const exactMatch = content.match(searchPattern);
  if(exactMatch) {
    const chinaEnding = \`    festivals: {
      title: '🎉 Festivaller',
    ...
    content = content.replace(searchPattern, chinaEnding + "\\n" + toInject);
  }
}
...
if (!content.includes('💰 {userGold} Altın')) {
  content = content.replace(/\{\/\* Sağ Taraf: Hoş Geldiniz Mesajı ve Çıkış Butonu \*\/\}[\s\S]*?<\/header>/, newRightSide + '\\n      </header>');
}
