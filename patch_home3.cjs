...
if (!content.includes('japan: {')) {
  const searchPattern = /    festivals: \{\s*title: '🎉 Festivaller',\s*desc: 'Geleneksel Çin Yeni Yılı[\s\S]*?\}\s*\}\s*\};/;
...
"  useEffect(() => {\n" +
"    if (user && user.email) {\n" +
"      const storedVisas = JSON.parse(localStorage.getItem(" + bt + "visas_" + dl + "{user.email}" + bt + ") || '[]');\n" +
...
"      const storedVisas = JSON.parse(localStorage.getItem(" + bt + "visas_" + dl + "{user.email}" + bt + ") || '[]');\n" +
"      setApprovedVisas(storedVisas);\n" +
"      const storedVal = localStorage.getItem(" + bt + "gold_" + dl + "{user.email}" + bt + ");\n" +
...
"      if (user && user.email) {\n" +
"        localStorage.setItem(" + bt + "gold_" + dl + "{user.email}" + bt + ", newGold.toString());\n" +
"      }\n" +
"      return newGold;\n" +
"    });\n" +
"  };";
