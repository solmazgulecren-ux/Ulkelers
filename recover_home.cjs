...
if (!content.includes('japan: {')) {
  content = content.replace(/bosnia: \{/, missingCountries + '\n  bosnia: {');
}
...
if (!content.includes('const [showMiniGames, setShowMiniGames]')) {
  content = content.replace("const [isTransitioning, setIsTransitioning] = useState(false);", 
...
// Look for "{/* Sağ Taraf: Hoş Geldiniz Mesajı ve Çıkış Butonu */}"
...
console.log('Successfully recovered lost user features in Home.jsx');
